...
export class PipelineWorkflow<T = unknown> implements Workflow {
  readonly type: WorkflowType = "pipeline";
  ...
  validate(): void {
    if (this.tasks.length === 0) {
      throw new Error("PipelineWorkflow requires at least 1 task");
    }
    if (this.tasks.length > this.config.maxTasks) {
      throw new Error(
        `PipelineWorkflow exceeds max tasks: ${this.tasks.length} > ${this.config.maxTasks}`,
      );
    }
    ...
    if (cycles) {
      throw new Error(`Pipeline contains cycles: ${cycles.join(" -> ")}`);
    }
  }
  ...
  async execute(context: TaskContext): Promise<WorkflowResult> {
    ...
    for (const task of this.tasks) {
      ...
      try {
        ...
        context.sharedStore.set(context.namespace, `result.${task.id}`, result);
